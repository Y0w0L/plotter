...
try:
    ...
    with log_file_context as log_file:
        ...
        for params in tqdm(param_list, desc="Overall Progress"):
            ...
            output_filename = f"/home/towa/alice3/hist/ce65sim202505/ce65sim202505_{pitch_name}_{chip_type}_{voltage}V_SeedThd{seed_threshold}e_NeighborThd{neighbor_threshold}e_{i}.root"

            if os.path.exists(output_filename):
                skip_message = f"----Skipping, file exists: {os.path.basename(output_filename)}\n"
                tqdm.write(skip_message.strip())
                if WRITE_LOG:                 
...
"""
    ...
    print(f"\nAll jobs finished.")
    if WRITE_LOG:
        print(f"You can check the detailed logs in '{log_filename}'.")